<?php
  /* cadastro dos motivos de vencimento das OS's
  Jonas da Silva Azevedo
  Criado: 21/01/2010 - 16:59
*/
  require("sessao.php");
  ...
  if($_GET['acao']=='cadastrar') {
    //editar
    if($_POST['edCodigo']!=''){
      $sql = "UPDATE motivos_vencimento_os SET motivo='" . $_POST['mmMotivo'] . "',descricao='" . $_POST['mmDescricao'] . "',";
      $sql = $sql . "ativo=" . $_POST['cbBxStatus'] . " WHERE codigo=" . $_POST['edCodigo'];
    }
    //cadastrar
    else {
      $sql = "INSERT INTO motivos_vencimento_os (motivo,descricao,ativo)";
      $sql = $sql . "VALUES ('". $_POST['mmMotivo'] . "','" . $_POST['mmDescricao'] ."'," . $_POST['cbBxStatus'] . ")";
    }
    $selMotivo = "SELECT * FROM motivos_vencimento_os WHERE motivo='" . $_POST['mmMotivo'] . "'";
    if($_POST['edCodigo']!=''){
    ...
    if($total==0){
      ...
      <script language="JavaScript">
        <!--
          alert("Motivo com este nome já cadastrado!")
        -->
      </script>
      <?php
    }
  ...
  if(isset($_GET['codigo'])) {
    $sqlCod = "SELECT * FROM motivos_vencimento_os m WHERE m.codigo=" . $_GET['codigo'];
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>Cadastro dos Motivos de Vencimento das OS's</title>
  </head>
  ...
  <body>
    <center>
      ...
      <p>
        ...
        <a href="principalMotivoVencimentoOS.php">Pesquisa</a>
      </p>
      <h2>CADASTRO DOS MOTIVO DE VENCIMENTO DAS OS's</h2>
    </center>
    <form name="cadastroMotivoVencimentoOS" method="POST" onSubmit="return validaDados(this)" action="<?php echo $PHP_SELF;?> ?acao=cadastrar<?php if(isset($tela)){echo '&opcao=' . $tela;} ?>">
      <table id="tblDadosMotivoVencimentoOS" border="0" align="center" cellpadding="10" cellpacing="1">
        <tr>
          <td>
        ...
        <tr>
          <td colspan="2">
            <b>Motivo:</b> <br> <textarea name="mmMotivo" rows="3" cols="92"><?php if($achouCodigo==true){echo $dadosRes[0]->motivo;}?></textarea>
          </td>
        </tr>
        <tr>
          <td colspan="2">
