...
$query = mysql_query("SELECT* FROM `cadastro` where `tr` = '$_GET[tr]'");
...
for($x=1;$x<cal_days_in_month(CAL_GREGORIAN, $mes, $ano)+1;$x++) {
	...
	$query2 = mysql_query("SELECT COUNT(`circuito`) FROM `tabelacptx` WHERE `data_e` = '$ano-$mes-$dia' and `tr` = '$_GET[tr]'");
